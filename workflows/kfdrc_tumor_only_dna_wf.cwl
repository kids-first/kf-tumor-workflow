...
doc: |
  ...
  ### Recommended file/param defaults:
   - `indexed_reference_fasta`: Homo_sapiens_assembly38.fasta with Homo_sapiens_assembly38.fasta.fai and Homo_sapiens_assembly38.dict
...
inputs:
  ...
  make_bamout: {type: 'boolean?', default: false, doc: "Should Mutect2 create a BAM output? Turning on this mode may result in serious performance cost for the caller."}
  ...
  i_flag: {type: ['null', {type: enum, name: i_flag, symbols: ["Y", "N"]}], doc: "Flag to intersect germline calls on padded regions. Use N if you want to skip this or have a WGS run"}
  ...
  indexed_reference_fasta: {type: 'File', secondaryFiles: [{pattern: ".fai", required: true}, {pattern: "^.dict", required: true}],
    doc: "Reference fasta to which the input reads were aligned. Must include FAI and DICT indicies",
  ...
  old_tumor_name: {type: 'string?', doc: "If `SM:` sample name in the align file is
  ...
  b_allele: {type: 'File?', secondaryFiles: [{pattern: ".tbi", required: true}], doc: "germline calls, needed for BAF.  GATK HC VQSR input
      recommended. Tool will prefilter for germline and pass if expression given",
    "sbg:suggestedValue": {class: File, path: 663e7c69f180a9177dd4212b, name: dbSNP_v153_ucsc-compatible.converted.vt.decomp.norm.common_snps.vcf.gz, secondaryFiles: [{class: File, path: 6509b6a37f6417197fc158fc, name: dbSNP_v153_ucsc-compatible.converted.vt.decomp.norm.common_snps.vcf.gz.tbi}]}}
  ...
  mutect2_retain_info: {type: 'string?', doc: "csv string with INFO fields that you want to
  ...
  gatk_filter_name: {type: 'string[]', doc: "Array of names for each filter tag to
      add, recommend: [\"GNOMAD_AF_HIGH\", \"AF_LOW\", \"AD_LOW\""}
  gatk_filter_expression: {type: 'string[]', doc: "Array of filter expressions to
      ...
      == 'PASS'\", \"FORMAT/AF < 0.01\", \"FORMAT/AD[:1] < 1\"]"}
...
outputs:
  ...
  mutect2_public_outputs: {type: 'File[]?', outputSource: mutect2/mutect2_public_outputs,
    doc: "Protected outputs, except MAF and VCF have had entries with soft FILTER
      values removed"}
  mutect2_bam: {type: 'File?', outputSource: mutect2/mutect2_bam, doc: "BAM generated
  ...
  ctrlfreec_pval: {type: 'File?', outputSource: controlfreec/ctrlfreec_pval, doc: 'Copy
  ...
  ctrlfreec_baf: {type: 'File?', outputSource: controlfreec/ctrlfreec_baf, doc: 'b
      allele frequency file'}
  ctrlfreec_info: {type: 'File?', outputSource: controlfreec/ctrlfreec_info, doc: 'Calculated
      inforamtion, like ploidy, if a range was given'}
  manta_pass_vcf: {type: 'File?', outputSource: manta/manta_pass_vcf, doc: 'VCF file
      with SV calls that PASS'}
  manta_prepass_vcf: {type: 'File?', outputSource: manta/manta_prepass_vcf, doc: 'VCF
